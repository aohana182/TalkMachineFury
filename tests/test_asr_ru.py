...
Requires: onnx-asr with GigaAM v3 installed, or vosk fallback.
...
class TestRussianASR:
    ...
    def test_backend_is_set(self, loaded_ru_model):
        ...
        assert backend in ("gigaam_v3", "vosk"), f"Unknown backend: {backend}"

    def test_rtf_under_1x(self, loaded_ru_model, ru_audio):
        """Real-time factor must be <1.0 on target hardware (i7-1355U)."""
        import time
        ...
        assert rtf < 1.0, f"RTF={rtf:.2f} — inference slower than real-time"
